      ...
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value as AppRole | "all")}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filteredUsers.map((user) => (
              <div
                key={user.roleId}
                className="p-4 bg-secondary/50 rounded-xl flex items-center justify-between gap-4 flex-wrap"
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  {user.role === "pending_member" && (
                    <Button
                      variant="warm"
                      size="sm"
                      onClick={() => handlePromote(user.user_id, user.role, "member")}
                    >
                  ...
                  {user.role === "candidate" && (
                    <Button
                      variant="warm"
                      size="sm"
                      onClick={() => handlePromote(user.user_id, user.role, "counselor")}
                    >
                  ...
                  {user.role === "member" && (
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 top-full mt-1 bg-card border rounded-lg shadow-lg py-1 hidden group-hover:block z-10 min-w-[140px]">
                        <button
                          onClick={() => handlePromote(user.user_id, user.role, "candidate")}
